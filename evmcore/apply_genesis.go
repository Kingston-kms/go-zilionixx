...
func ApplyGenesis(statedb *state.StateDB, g zilionixx.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g zilionixx.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g zilionixx.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
